...
- ✅ **TenantSelect usklađen sa oci-ui** (2026-03-13): Custom GroupedSelect dropdown vizuelno odgovara Mantine Select-u. Iste širine (16.25rem tenant, max-w-40 subscription), grupisanje po organizaciji, check ikona za selektovano, subscription prikazuje samo subscriptionID (kao oci-ui). Isti API endpointi (`/codebook/tenants`, `/codebook/subscriptions`). Tenant sortiranje alphabetically sa "name not defined" na kraju (kao oci-ui).
- ✅ **Tenant-based data filtering** (2026-03-13): Sve stranice i widgeti filtriraju podatke po selektovanom tenantu. Client-side filtriranje (backend API ne podržava tenant query parametre). Dva pristupa: (1) **Direktno filtriranje** (`d.tenant?.ocid === tenant.ocid`) za SlaDefinitionsTable, SlaTimelinePage, SlaReportForm, SlaReportScheduleFormPage. (2) **Indirektno filtriranje** putem novog `useTenantDefinitionIds` hooka (`Set<string>` definicija za tenant) za BreachTable, ScheduleTable, StoredReportTable, NotificationTable. React Query cache sharing (`slaKeys.all()`). Dvostepeni filter pattern: tenant scoping → user filters. Timeline auto-select first definition na tenant change.
- ✅ **Tenant pre-selection na SLA create formi** (2026-03-13): Dropdown "Zakupac (Organizacija)" na `/sla/create` stranici se automatski popunjava iz selektovanog tenanta u header-u (`useTenantContext()`). Match strategija u `SlaFormPage.tsx` (po prioritetu): (1) `org.name === tenant.name`, (2) `org.id === tenant.id`, (3) `org.id === tenant.organization.id`. Radi samo u create modu (ne u edit modu). `userSelectedTenant` boolean flag prati da li je korisnik ručno izabrao — kad se header tenant promeni, forma se automatski ažurira (osim ako je korisnik ručno selektovao). "Form context pre-fill" pattern dokumentovan u `docs/oci-ui-code-patterns.md` §12.
- ✅ **Language picker (oci-ui aligned)** (2026-03-13): `LanguagePicker` komponenta za prebacivanje Ćirilica/Latinica, po oci-ui `language-picker.tsx` patternu. Custom dropdown sa inline SVG zastavom Srbije. Placement: SlaNavigation (između NotificationBell i Logout) + LoginPage (top-right). Poziva `i18n.changeLanguage()`, čuva u localStorage key `'language'` via `LocalStorageKeys.Language`. Konstante u `constants/index.ts`, `i18n.ts` koristi konstante umesto string literala. UI tim će zameniti sa Mantine Menu + `country-flag-icons`. Dokumentovano u `docs/oci-ui-code-patterns.md` §13.
- ✅ **Table alignment — semantic `<table>` + column sorting** (2026-03-13): Svih 5 widgeta (SlaDefinitionsTable, BreachTable, ScheduleTable, StoredReportTable, NotificationTable) konvertovano sa `div.grid.grid-cols-12` layout-a na proper `<table>`/`<thead>`/`<tbody>`/`<tr>`/`<th>`/`<td>` HTML elemente. Nova `SortIndicator` komponenta (`src/components/ui/SortIndicator.tsx`) sa ↑↓ arrows. Sort state + toggleSort u svakom widgetu. `filteredX` useMemo: filter → sort. `overflow-x-auto` wrapper. Vizuelno usklađeno sa oci-ui MRT tabelama. Chipovi (Period, Status, Severity, Frequency) zadržani. Dokumentovano u `docs/oci-ui-code-patterns.md` §9.
- ✅ **Dead code cleanup** (2026-03-13): Uklonjeno ~33 nekorišćenih eksporta/fajlova. Obrisano 7 fajlova (axios.ts + 6 mutation hookova). Uklonjeno 5 dead detail-fetch hookova, 15 dead servisnih metoda, 3 dead tipa, 10 dead queryKey entries, useOrganizationContext export, 2x console.log. Un-exported 3 interne util funkcije. Cilj: smanjenje codebase-a za lakšu integraciju u oci-ui.
...
| G-06 | ~~Email retry mehanizam~~ | ~~Backend~~ | ~~VISOK~~ | ~~4-6h~~ | ✅ **KOMPLETNO IMPLEMENTIRANO** (2026-03-12): **Phase 1** — Inline exponential backoff u sva 4 MailerService implementacije. **Phase 2** — Scheduled cleanup: `EmailSendLog` entity, `EmailSendLogRepository`, `EmailSendLogService` (sendEmailWithPersistence + processRetryableEmails), `EmailRetryScheduler` (@Scheduled 5min + @SchedulerLock + SchedulerToggleService). `SlaNotificationService` prebačen na EmailSendLogService (prvi consumer). Flyway: dev V12, prod V6. **Pending**: ~~(1) `email.retry.scheduled` seed data u `scheduler_settings`~~ ✅ Flyway dev V13, prod V7. (2) Inkrementalna migracija ostalih oci-monitor pozivalaca (BudgetNotificationService, SubscriptionNotificationService, CommitmentNotificationService, CostReportsService, MetricsNotificationEventListener) sa direktnog `mailerService` na `emailSendLogService.sendEmailWithPersistence()`. |
...
| G-16 | ~~Notifikacije za bitne evente~~ | ~~Backend + Frontend~~ | ~~7-9h~~ | ✅ **KOMPLETNO IMPLEMENTIRANO** (2026-03-12): **Backend**: `sla_event_log` tabela, `SlaEventType` enum (7 vrednosti), `SlaEventLog` entity, `SlaEventLogService` (write-side, oci-api), `SlaNotificationController` (read-side REST, oci-api), `SlaEventNotificationScheduler` (email scheduler, oci-monitor), `SlaNotificationService.sendEventNotification()` (email via EmailSendLogService). logEvent pozivi u SlaService, SlaReportScheduleManagementService, SlaReportGenerationService. Flyway: dev V14, prod V8. **Frontend**: `NotificationBell` (badge + dropdown), `SlaNotificationListPage` (/sla/notifications, tabela + filteri + pagination), `slaNotificationService`, tipovi, ruta u App.tsx. |
