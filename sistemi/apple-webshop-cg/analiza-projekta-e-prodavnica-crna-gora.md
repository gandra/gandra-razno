...
| **4** | **Da li API ima rate limiting?** Koliko poziva/min? **Da li `getItems` podržava pagination** (npr. page/offset/limit parametri) ili uvijek vraća SVE artikle u jednom odgovoru? Konkretno za `getItems` — ako imamo 300 artikala, da li dolaze svih 300 odjednom ili možemo tražiti po stranicama? | ERP/Abacus | 🟡 Srednji | Koristimo API za periodični sync (svi artikli) i za real-time checkout check (2-3 artikla). Moramo znati: (1) koliko poziva smijemo poslati, (2) koliki je payload kad tražimo sve artikle. |
