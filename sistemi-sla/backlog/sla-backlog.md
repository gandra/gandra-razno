...
| G-19 | Tenant pre-selection na SLA create formi | 2026-03-13 | "Zakupac (Organizacija)" dropdown na `/sla/create` automatski pre-populated iz header TenantContext-a. Match strategija (po prioritetu): (1) `org.name === tenant.name`, (2) `org.id === tenant.id`, (3) `org.id === tenant.organization.id`. Name-based match je primarni jer header tenant name treba da odgovara organizacija dropdown-u. `userSelectedTenant` flag prati ručnu selekciju — kad se header tenant promeni, forma se automatski ažurira osim ako korisnik ručno izabere. Samo create mode (ne edit). |
...
| G-21 | Table alignment — semantic `<table>` + column sorting | 2026-03-13 | Svih 5 widgeta konvertovano sa `div.grid.grid-cols-12` na proper `<table>`/`<thead>`/`<tbody>`/`<tr>`/`<th>`/`<td>` HTML elemente. Sortable column headers sa `SortIndicator` komponentom (`src/components/ui/SortIndicator.tsx`). Sort state (`sortField`/`sortDirection`) + `toggleSort` handler u svakom widgetu. `filteredX` useMemo: filter + sort. `overflow-x-auto` wrapper za responsive. Vizuelno usklađeno sa oci-ui MRT tabelama (bg-muted/50 header, hover:bg-muted/30 rows). Chipovi zadržani (Period, Status, Severity, Frequency). Docs: §9. |
| G-22 | Dead code cleanup | 2026-03-13 | Uklonjeno ~33 nekorišćenih eksporta/fajlova radi smanjenja codebase-a za lakšu integraciju u oci-ui. **Obrisani fajlovi (7)**: `src/lib/axios.ts` (zamenjen sa `axiosInstance.ts`), 6 mutation hook fajlova (useSlaMutations, useSlaBreachMutations, useSlaExcludedDowntimeMutations, useSlaNotificationMutations, useSlaReportScheduleMutations, useStoredSlaReportMutations) — nikad importovani. **Uklonjeni dead hook eksporti (5)**: useResource, useOrganization, useOciQuery, useMetricName, useMetricNamespace. **Uklonjene dead servisne metode (15)**: getById iz 5 servisa, getById/getActiveBySlaDefinition/getUpcoming iz slaExcludedDowntimeService, 7 duplicate metoda (Stored Reports CRUD) iz slaReportScheduleService. **Uklonjeni dead tipovi (3)**: GenerateReportRequest, ReportFormat, SlaReportScheduleFormData. **Uklonjeni dead queryKeys (10)**: detail entries iz 5 key factories, detail/active/upcoming iz slaExcludedDowntimeKeys, detail/latest iz storedSlaReportKeys. **Ostalo**: useOrganizationContext (nikad importovan), un-export formatNumber/getCurrencySymbol/formatCurrency (interno korišćeni), 2x console.log iz SlaFormPage. |
