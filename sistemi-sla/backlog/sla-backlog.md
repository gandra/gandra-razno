...
| G-16 | SLA Notifikacije — Backend (Event Log + Scheduled Email) | 2026-03-12 | `sla_event_log` tabela, `SlaEventType` enum, `SlaEventLog` entity (oci-library). **oci-api**: `SlaEventLogService` (write-side logEvent), `SlaEventLogRepository`, `SlaNotificationController` (REST: GET/undismissed/count/dismiss), `SlaEventLogDto`, `SlaEventLogMapper`. **oci-monitor**: `SlaEventNotificationScheduler` (@Scheduled 5min, ShedLock, SchedulerToggleService), `SlaNotificationService.sendEventNotification()` via EmailSendLogService, `SlaEventLogRepository`. **Integracioni pozivi**: SlaService (deactivate, delete, acknowledge, resolve), SlaReportScheduleManagementService (schedule status), SlaReportGenerationService (report generated). Flyway: dev V14, prod V8. **Frontend TODO**: NotificationBell, NotificationDropdown, NotificationListPage. |
...
| G-16 | ~~Notifikacije za bitne evente~~ | ~~7-9h~~ | ~~SREDNJI~~ | ✅ DONE (backend). Frontend (NotificationBell, dropdown, list page) ostaje TODO. |
| G-16-FE | SLA Notifikacije — Frontend (NotificationBell, dropdown, list page) | 4-6h | SREDNJI | BACKLOG |
